...
public abstract class BaseDAO<T> {
    ...
    public final String URL = "jdbc:postgresql://localhost:5432/interactiveZoneDB";
        ...
        if( params!= null && params.length>0 ){
            for ( int i = 0; i < params.length; i++ ) {
                ...
                if( params[i].getClass() == Date.class ){
                    System.out.println("Flag_1 params[" + i +"]" + params[i] + "; i : " + i  );
                    psmt.setObject(i+1,params[i],Types.TIMESTAMP);
                }else {
                    System.out.println("Flag_2 params[" +i +"]" + params[i] + "; i : " + i  );
        ...
        Class clazz = obj.getClass();
            ...
            if( field != null ){//  如果 field不为null, 就直接setValue了； 这里 有一个问题， 得到的值 的类型，和被setValue的类型 不匹配；
                ...
                if( isMyType(typeName) ){
                    ...
                    Constructor constructor = typeNameClass.getDeclaredConstructor(java.lang.Integer.class);//获取构造器
    ...
    protected T load(String sql , Object... params){
        System.out.println("进入 返回单个实体 对象的查询 sql : " + sql );
        try {
            conn = getConn() ;
            psmt = conn.prepareStatement(sql);
            setParams(psmt,params);
